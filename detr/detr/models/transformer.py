...
class Transformer(nn.Module):
    ...
    def forward(self, src, mask, query_embed, pos_embed, latent_input=None, proprio_input=None, additional_pos_embed=None, debug=False, tgt=None):
        ...
        if debug:
            ...
            # show the pos_embed
            plt.figure()
            plt.imshow(pos_embed[:, 0, :].detach().cpu().numpy())
            plt.title('pos_embed')
        # plt.show()

        # print("mask", mask)
        memory = self.encoder(src, src_key_padding_mask=mask, pos=pos_embed, debug=debug)
        # plot the memory
        if debug:
            plt.figure()
            plt.imshow(memory[:, 0, :].detach().cpu().numpy())
            plt.title('memory')
            # plt.show()
            print('memory shape', memory.shape)
        hs = self.decoder(tgt, memory, memory_key_padding_mask=mask,
                          pos=pos_embed, query_pos=query_embed, debug=debug)
            # plot the hs
        if debug:
            plt.figure()
            plt.imshow(hs[0, :, 0, :].detach().cpu().numpy())
            plt.title('hs')
            plt.show()
...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
                 activation="relu", normalize_before=False) -> None:
        super().__init__()
    ...
    def forward_post(self,
                     ...
                     src_key_padding_mask: Optional[Tensor] = None,
                     pos: Optional[Tensor] = None, debug = False):
        ...
        if debug:
            plt.figure()
            plt.imshow(src2[:, 0, :].detach().cpu().numpy())
            plt.title('src2')
    ...
    def forward(self, src,
        ...
        return self.forward_post(src, src_mask, src_key_padding_mask, pos, debug=debug)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_post(self, tgt, memory,
                     ...
                     memory_key_padding_mask: Optional[Tensor] = None,
                     pos: Optional[Tensor] = None,
                     query_pos: Optional[Tensor] = None, debug = False):
        ...
        if debug:
            ...
            # plot the q
            plt.figure()
            plt.imshow(q[:, 0, :].detach().cpu().numpy())
            plt.title('q')
        ...
        if debug:
            print('tgt (new) shape', tgt.shape)
            # plot new tgt
            plt.figure()
            plt.imshow(tgt[:, 0, :].detach().cpu().numpy())
            plt.title('tgt II')
        tgt = self.norm1(tgt)
    ...
    def forward(self, tgt, memory,
        ...
        return self.forward_post(tgt, memory, tgt_mask, memory_mask,
                                 tgt_key_padding_mask, memory_key_padding_mask, pos, query_pos,  debug)
