...
def write_markdown(rows: list[dict[str, Any]], path: Path, args: argparse.Namespace, csv_path: Path, json_path: Path) -> None:
    lines = [
        ...
        "4. After all trials, assign `pair_id` into metadata with `assign_rollout_pair_ids.py` or manually verify against this manifest.",
        "5. For insertion, fill `success`, `stopped_early`, `bounce_count`, and `retry_count` before claiming outcome metrics.",
        "6. Run `eval_tac_quality_real_rollouts.py`; only non-synthetic paired logs count as real evidence.",
        "",
        "## Planned Trials",
        "",
