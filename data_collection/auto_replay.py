...
用法:
    # 干跑模式 (不连机器人, 只检查轨迹)
    python auto_replay.py --traj trajectories/wipe_pos_000.npy --save_dir ./data --dry_run

    # 真机回放单条
    python auto_replay.py --traj trajectories/wipe_pos_000.npy --save_dir ./data

    # 批量回放
    python auto_replay.py --traj_dir ./trajectories/ --filter "wipe_pos" --save_dir ./data
    ...
    for step in range(T):
        ...
        if config.USE_INTERPOLATION and env.last_pose_quat is not None:
            ...
            for k, p in enumerate(path, start=1):
                env.arm.rm_movep_canfd(p.tolist(), 0)
                t_target = t_base + k * config.CTRL_DT
                while time.perf_counter() < t_target:
                    time.sleep(0.001)
        else:
            env.arm.rm_movep_canfd(target_quat.tolist(), 0)
