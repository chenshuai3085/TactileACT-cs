...
class RobotiqGripperServerNoROS(GripperServer):
    ...
    def move(self, position):
        """移动到指定位置 (0-235): 0 完全打开，235 完全关闭"""
        position = int(max(0, min(235, position)))
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("Usage: python robotiq_gripper_server_no_ros.py /dev/ttyUSB0")
        sys.exit(1)
    ...
    while True:
        ctr = input("Enter position (0-235): ")
        if not ctr.isdigit():
            print("Exiting...")
            break
        print("\n4. Moving...")
