...
class Config:
    ...
    # 现在 Sigma7 按钮幅度用于控制夹爪，采集开始改由 S 键触发，因此默认不再用 Sigma7 按钮结束 episode。
    END_ON_TELEOP_DISABLE = False
    # Sigma7 按钮现在用于夹爪幅度，默认不再把它作为遥操作 enabled 门控。
    TELEOP_ENABLE_WITH_SIGMA7_BUTTON = False
    # ===== 状态存储类型：可选 "eef" / "joint" =====
    ...
    # None 自动判断按钮范围：0~1 直接归一化，0~235 视作夹爪位置，>235 按 0~255 归一化。
    GRIPPER_BUTTON_INPUT_MAX = None
    GRIPPER_BUTTON_USE_ABS = True
    GRIPPER_BUTTON_INVERT = False
    GRIPPER_COMMAND_HZ = 15.0
    GRIPPER_SEND_DELTA = 1.0
...
def _flatten_numeric_values(value):
    ...
    if isinstance(value, dict):
        ...
        for k in sorted(value.keys(), key=str):
            out.extend(_flatten_numeric_values(value[k]))
        return out
...
def _extract_button_value(buttons, index):
...
def _button_value_to_gripper_pos(value, cfg):
    raw = abs(float(value)) if getattr(cfg, "GRIPPER_BUTTON_USE_ABS", True) else float(value)
    in_min = float(getattr(cfg, "GRIPPER_BUTTON_INPUT_MIN", 0.0))
    in_max = getattr(cfg, "GRIPPER_BUTTON_INPUT_MAX", None)
    ...
    if in_max is None:
        ...
        ratio = (raw - in_min) / max(float(in_max) - in_min, 1e-6)
...
class SimplePIHEnv:
    ...
    def __init__(self, config: Config):
        ...
        self._gripper_target_pos = self.current_gripper_pos
        self._gripper_last_sent = None
        self._gripper_stop_event = threading.Event()
        self._gripper_lock = threading.Lock()
        self._gripper_thread = None
        if getattr(config, "GRIPPER_ENABLED", False):
            try:
                GripperClass = _load_local_gripper_class()
                self.gripper = GripperClass(getattr(config, "GRIPPER_PORT", "/dev/ttyUSB1"))
                if getattr(config, "GRIPPER_ACTIVATE_ON_INIT", True):
                    self.gripper.activate_gripper()
                if getattr(config, "GRIPPER_RESET_OPEN", True):
                    self.set_gripper_target(getattr(config, "GRIPPER_OPEN_POS", 0.0), force=True)
                self._gripper_thread = threading.Thread(target=self._gripper_worker, daemon=True)
                self._gripper_thread.start()
                print(
                    f"✅ 夹爪已初始化: {getattr(config, 'GRIPPER_PORT', '/dev/ttyUSB1')} "
                    f"range={getattr(config, 'GRIPPER_OPEN_POS', 0.0)}~{getattr(config, 'GRIPPER_CLOSE_POS', 235.0)}"
                )
            except Exception as e:
                print(f"⚠️  夹爪初始化失败，继续不采集夹爪维度: {e}")
                self.gripper = None
    ...
    def reset(self):
        ...
        if self.gripper is not None and getattr(self.cfg, "GRIPPER_RESET_OPEN", True):
            self.set_gripper_target(getattr(self.cfg, "GRIPPER_OPEN_POS", 0.0), force=True)
    ...
    def set_gripper_target(self, position, force=False):
    ...
    def update_gripper_from_buttons(self, buttons):
        ...
        raw = _extract_button_value(buttons, getattr(self.cfg, "GRIPPER_BUTTON_INDEX", 0))
        target = _button_value_to_gripper_pos(raw, self.cfg)
        ...
        if getattr(self.cfg, "GRIPPER_PRINT_BUTTONS", False):
            ...
            if now - last_t >= interval:
                print(f"[gripper] buttons={buttons} raw={raw:.4f} target={target:.1f}")
    ...
    def _gripper_worker(self):
        ...
        min_delta = float(getattr(self.cfg, "GRIPPER_SEND_DELTA", 1.0))
        last_send_t = 0.0
        while not self._gripper_stop_event.is_set():
            with self._gripper_lock:
                target = float(self._gripper_target_pos)
                last_sent = self._gripper_last_sent
            now = time.perf_counter()
            due = (now - last_send_t) >= min_period
            changed = last_sent is None or abs(target - last_sent) >= min_delta
            if due and changed and self.gripper is not None:
                try:
                    self.gripper.move(target)
                    with self._gripper_lock:
                        self._gripper_last_sent = target
    ...
    def step(self, spacemouse_action, enabled: bool=True):
        ...
        if USE_SIGMA7 and CONTROL_MODE == "abs":
            # ===== Sigma7 绝对控制 =====
            # 1) 是否使用 Sigma7 按钮作为遥操作门控。默认 False，因为该按钮幅度用于控制夹爪。
            if getattr(self.cfg, "TELEOP_ENABLE_WITH_SIGMA7_BUTTON", False):
    ...
    def _get_obs(self):
        ...
        if "joint" in state_types:
            ...
            if self.gripper is not None and getattr(self.cfg, "GRIPPER_ENABLED", False):
    ...
    def close(self):
        ...
        if self.gripper is not None and hasattr(self.gripper, "serial_port"):
            try:
                self.gripper.serial_port.close()
            except Exception:
                pass
...
def collect_episode(env: SimplePIHEnv, episode_idx: int, save_path: Path):
    ...
    try:
        while not done and not stop_flag[0]:
            ...
            enabled = True
            if getattr(env.cfg, "TELEOP_ENABLE_WITH_SIGMA7_BUTTON", False) and hasattr(env.spacemouse, "is_enabled"):
                try:
            ...
            if enabled and record_flag[0]:
                ...
                if record_step_idx >= warmup_steps:
                    ...
                    if "joint_abs" in action_types:
                        ...
                        if env.gripper is not None and getattr(env.cfg, "GRIPPER_ENABLED", False):
    ...
    if env.gripper is not None and "proprio_joint" in observations[0]:
        _build_and_store(
    ...
    if env.gripper is not None and action_buffers.get("joint_abs"):
        _build_and_store(
            "actions/gripper_pos",
...
def main():
    ...
    parser.add_argument(
        ...
        type=lambda x: x.lower() == "true",
        default=None,
        help="遥操作从开->关时自动结束episode（true/false，默认用 Config；夹爪按钮控制时建议 false）",
    )
    parser.add_argument(
        "--teleop-enable-with-sigma7-button",
        type=lambda x: x.lower() == "true",
        default=None,
        help="是否仍用 Sigma7 按钮作为遥操作使能（true/false，默认 false；夹爪按钮控制时应保持 false）",
    )
    parser.add_argument(
        "--gripper",
        type=lambda x: x.lower() == "true",
        default=None,
        help="是否启用夹爪控制/采集（true/false，默认用 Config.GRIPPER_ENABLED）",
    )
    parser.add_argument(
        "--gripper-port",
        type=str,
    ...
    if args.teleop_enable_with_sigma7_button is not None:
        config.TELEOP_ENABLE_WITH_SIGMA7_BUTTON = args.teleop_enable_with_sigma7_button
    ...
    print(f"   - 夹爪控制/采集: {'✅ 启用' if config.GRIPPER_ENABLED else '❌ 禁用'}")
