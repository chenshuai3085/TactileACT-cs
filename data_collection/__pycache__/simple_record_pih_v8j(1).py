...
Usage examples:
    # 正常采集
    python3 simple_record_pih_v8j.py --episodes 1

    # 开启可视化，显示裁剪后的模型输入画面（266x200）
    python3 simple_record_pih_v8j.py --visualize --visualize-cropped

    # 裁剪后窗口放大到 640x480 便于观察（单独加 --visualize-cropped-win 也可以）
    python3 simple_record_pih_v8j.py --visualize --visualize-cropped --visualize-cropped-win
    python3 simple_record_pih_v8j.py --visualize-cropped-win
...
USE_SIGMA7 = True
CONTROL_MODE = "abs"  # "inc" 增量
# True:  打印详细性能指标（movep/obs/cam/tac 耗时）模式，后面绝对控制时会用 "abs"
...
class Config:
    ...
    FT_SCALE = np.array([0.1, 1])  # 260515 260519 2pin
    ...
    DATA_DIR = Path("/home/czy/dataset/pih/260625_v8j_caheiban/peg_in_hole_0519")  # 数据保存路径

    # 当遥操作设备（如 Sigma7）从“启用->禁用”时，是否自动结束当前 episode。
    # 夹爪改为使用 Sigma7 按钮幅度控制后，默认不再用按钮结束采集。
    END_ON_TELEOP_DISABLE = False
    # ===== 状态存储类型：可选 "eef" / "joint" =====
    ...
    # 夹爪控制：Sigma7 按钮幅度 -> Robotiq 目标位置，0 完全打开，235 完全关闭。
    GRIPPER_ENABLED = True
    GRIPPER_PORT = "/dev/ttyUSB1"
    GRIPPER_OPEN_POS = 0.0
    GRIPPER_CLOSE_POS = 235.0
    GRIPPER_RESET_OPEN = True
    GRIPPER_ACTIVATE_ON_INIT = True
    GRIPPER_COMMAND_HZ = 15.0
    GRIPPER_MIN_SEND_DELTA = 1.0
    GRIPPER_APPEND_TO_JOINT = True
    GRIPPER_BUTTON_INDEX = 0
    GRIPPER_BUTTON_INPUT_MIN = 0.0
    # None 表示自动判断：0~1 归一化，0~235 视为夹爪位置，>235 按 0~255 归一化。
    GRIPPER_BUTTON_INPUT_MAX = None
    GRIPPER_BUTTON_INVERT = False
    GRIPPER_BUTTON_USE_ABS = False
    GRIPPER_PRINT_BUTTONS = False
    GRIPPER_PRINT_INTERVAL_SEC = 0.5
    ...
    CAMERAS = {
        "global": {
            ...
            "dim": (640, 480),
            "fps": 30,
            "exposure": 15000, # 0515 05000 0519 15000
        ...
        "wrist": {
            ...
            "exposure": 15000,
            "crop": lambda x: x[:, :],
            # "exposure": 15000,  # 0515 0519 2pin
            # "crop": lambda x: x[50:320, 150:510],# 0515 0519 2pin
        }
    }
    TACTILE_SENSORS = {
        "left": {
    ...
    RESET_POSE = [0.390345, -0.005217, 0.28837, 3.141, -0.007, -4.350]
...
def _load_robotiq_gripper_class():
    ...
    return module.RobotiqGripperServerNoROS


def _flatten_numeric_values(value):
    """从 dict/list/ndarray/标量中提取数值，便于兼容不同 Sigma7 返回格式。"""
    if value is None:
        return []
    if isinstance(value, dict):
        numbers = []
        for key in sorted(value.keys(), key=str):
            numbers.extend(_flatten_numeric_values(value[key]))
        return numbers
    if isinstance(value, np.ndarray):
        return [float(x) for x in value.astype(float).ravel().tolist()]
...
def _extract_gripper_button_value(buttons, index=0):
    values = _flatten_numeric_values(buttons)
    if not values:
        return 0.0
    index = int(index)
    if index < 0:
...
def _map_button_value_to_gripper_pos(value, cfg):
    raw = abs(float(value)) if getattr(cfg, "GRIPPER_BUTTON_USE_ABS", False) else float(value)
    in_min = float(getattr(cfg, "GRIPPER_BUTTON_INPUT_MIN", 0.0))
    in_max_cfg = getattr(cfg, "GRIPPER_BUTTON_INPUT_MAX", None)
    open_pos = float(getattr(cfg, "GRIPPER_OPEN_POS", 0.0))
    close_pos = float(getattr(cfg, "GRIPPER_CLOSE_POS", 235.0))

    if in_max_cfg is None:
        if raw <= 1.0:
            ratio = raw
        elif raw <= close_pos:
            ratio = raw / max(close_pos, 1e-6)
        else:
            ratio = raw / 255.0
    else:
        in_max = float(in_max_cfg)
        denom = max(in_max - in_min, 1e-6)
        ratio = (raw - in_min) / denom

    ratio = float(np.clip(ratio, 0.0, 1.0))
    if getattr(cfg, "GRIPPER_BUTTON_INVERT", False):
        ratio = 1.0 - ratio
    return float(np.clip(open_pos + ratio * (close_pos - open_pos), open_pos, close_pos))


class AsyncGripperController:
    """后台节流发送夹爪命令，避免串口 sleep 拖慢主控制循环。"""

    def __init__(self, cfg):
        self.cfg = cfg
        self.enabled = bool(getattr(cfg, "GRIPPER_ENABLED", False))
        self.target_pos = float(getattr(cfg, "GRIPPER_OPEN_POS", 0.0))
        ...
        GripperClass = _load_robotiq_gripper_class()
        self._gripper = GripperClass(getattr(cfg, "GRIPPER_PORT", "/dev/ttyUSB1"))
        if getattr(cfg, "GRIPPER_ACTIVATE_ON_INIT", True):
            self._gripper.activate_gripper()
        if getattr(cfg, "GRIPPER_RESET_OPEN", True):
            self.set_target(getattr(cfg, "GRIPPER_OPEN_POS", 0.0), force=True)

        self._thread = threading.Thread(target=self._run, name="AsyncGripperController", daemon=True)
        self._thread.start()
        print(
            "✅ 夹爪已初始化: "
            f"port={getattr(cfg, 'GRIPPER_PORT', '/dev/ttyUSB1')}, "
            f"range={getattr(cfg, 'GRIPPER_OPEN_POS', 0.0)}~{getattr(cfg, 'GRIPPER_CLOSE_POS', 235.0)}"
        )

    def set_target(self, position, force=False):
        if not self.enabled:
            return
        open_pos = float(getattr(self.cfg, "GRIPPER_OPEN_POS", 0.0))
        close_pos = float(getattr(self.cfg, "GRIPPER_CLOSE_POS", 235.0))
        pos = float(np.clip(float(position), open_pos, close_pos))
        with self._lock:
            self.target_pos = pos
    ...
    def _run(self):
        min_period = 1.0 / max(float(getattr(self.cfg, "GRIPPER_COMMAND_HZ", 15.0)), 1e-6)
        min_delta = float(getattr(self.cfg, "GRIPPER_MIN_SEND_DELTA", 1.0))
        while not self._stop_event.is_set():
            with self._lock:
                target = float(self.target_pos)
            now = time.perf_counter()
            due = (now - self._last_send_t) >= min_period
            changed = self._last_sent is None or abs(target - self._last_sent) >= min_delta
            if self._gripper is not None and due and changed:
                try:
                    self._gripper.move(target)
                    self._last_sent = target
                    self._last_send_t = time.perf_counter()
                except Exception as e:
                    print(f"⚠️  夹爪命令发送失败: {e}")
                    self._last_send_t = time.perf_counter()
            time.sleep(0.005)
    ...
    def close(self):
        ...
        if self._gripper is not None and hasattr(self._gripper, "serial_port"):
            try:
                self._gripper.serial_port.close()
            except Exception:
                pass
...
class SimplePIHEnv:
    ...
    def __init__(self, config: Config):
        ...
        # IK 求解器（movej_canfd 需要先算关节角再发送）
        self._ik_algo = Algo(rm_robot_arm_model_e.RM_MODEL_RM_75_E,
                             rm_force_type_e.RM_MODEL_RM_SF_E)
        ...
        # 5. 初始化夹爪（可选）
        self.gripper = None
        self.current_gripper_pos = float(getattr(config, "GRIPPER_OPEN_POS", 0.0))
        if getattr(config, "GRIPPER_ENABLED", False):
            try:
                self.gripper = AsyncGripperController(config)
                self.current_gripper_pos = self.gripper.get_position()
            except Exception as e:
                print(f"⚠️  夹爪初始化失败，关闭夹爪维度并继续只采机械臂数据: {e}")
                self.gripper = None
                self.cfg.GRIPPER_ENABLED = False
        ...
        if getattr(config, 'VISUALIZE_SENSORS', False):
            try:
                ...
                self._sensor_visualizer = SensorVisualizer(
                    self,
    ...
    def reset(self):
        ...
        if self.gripper is not None and getattr(self.cfg, "GRIPPER_RESET_OPEN", True):
            self.gripper.set_target(getattr(self.cfg, "GRIPPER_OPEN_POS", 0.0), force=True)
            self.current_gripper_pos = self.gripper.get_position()
    ...
    def step(self, spacemouse_action, enabled: bool=True):
        ...
        self._cmd_gripper_pos_t = self.current_gripper_pos
        ...
        if self.cfg.USE_INTERPOLATION:
            # 关节空间插值：先对目标位姿做IK得到目标关节角，再linspace插值
            cmd_pose_interp = self.desired_tcp_pose.copy()
            if self.admittance is not None and ft_now is not None:
                pose_delta = self.admittance.compute_delta(
                    ft_now.copy(), control_axis=[0, 1, 2])
                cmd_pose_interp[:3] += pose_delta[:3]
            ik_params_interp = rm_inverse_kinematics_params_t(
                snap["joint"].tolist(), cmd_pose_interp.tolist(), 1)
            ik_ret_interp = self._ik_algo.rm_algo_inverse_kinematics(ik_params_interp)
            if ik_ret_interp[0] == 0:
                joint_target = np.array(ik_ret_interp[1])
                joint_start = snap["joint"].copy()
                path = np.linspace(joint_start, joint_target, self.cfg.N_SUBSTEPS + 1)[1:]

                t_base = start_time
                movep_times = []
                for k, j in enumerate(path, start=1):
                    t_movep_start = time.perf_counter()
                    self.arm.rm_movej_canfd(j.tolist(), True, 0)
                    ...
                    while True:
                        ...
                        if t_target - now <= 0:
                            break
                        time.sleep(min(t_target - now, 0.001))
            else:
                # IK失败回退：末端空间插值 + movep
                ...
                for k, p in enumerate(path, start=1):
                    t_movep_start = time.perf_counter()
                    self.arm.rm_movep_canfd(p.tolist(), True)
                    t_movep_end = time.perf_counter()
                    movep_times.append((t_movep_end - t_movep_start) * 1000)
                    t_target = t_base + k * self.cfg.CTRL_DT
                    while True:
            ...
            if PRINT_PERFORMANCE:
                if movep_times:
                    ...
                    print(f"[perf] movej: {len(movep_times)}子步 avg={avg_movep:.2f}ms max={max_movep:.2f}ms")
        else:
            # 不插值，直接发送目标关节角（movej_canfd）
            cmd_pose = self.desired_tcp_pose.copy()

            # 导纳控制补偿（如果启用，在IK之前加到末端位姿上）
            if self.admittance is not None and ft_now is not None:
                pose_delta = self.admittance.compute_delta(
                    ft_now.copy(), control_axis=[0, 1, 2])
                cmd_pose[:3] += pose_delta[:3]
            # IK: 末端位姿 → 关节角
            ik_params = rm_inverse_kinematics_params_t(
                snap["joint"].tolist(), cmd_pose.tolist(), 1)
            ik_ret = self._ik_algo.rm_algo_inverse_kinematics(ik_params)
            # 发送命令（计时）
            if PRINT_PERFORMANCE:
                t_movep_start = time.perf_counter()
            if ik_ret[0] == 0:
                self.arm.rm_movej_canfd(ik_ret[1], True, 0)
            else:
                self.arm.rm_movep_canfd(
                    poseuler_to_posquat(cmd_pose, order="xyz").tolist(), True)
            if PRINT_PERFORMANCE:
                t_movep_end = time.perf_counter()
    ...
    def update_gripper_from_buttons(self, buttons):
        if self.gripper is None or not getattr(self.cfg, "GRIPPER_ENABLED", False):
            return self.current_gripper_pos
        raw = _extract_gripper_button_value(
            buttons,
            index=getattr(self.cfg, "GRIPPER_BUTTON_INDEX", 0),
        )
        pos = _map_button_value_to_gripper_pos(raw, self.cfg)
        self.gripper.set_target(pos)
        self.current_gripper_pos = self.gripper.get_position()

        if getattr(self.cfg, "GRIPPER_PRINT_BUTTONS", False):
            now = time.perf_counter()
            last_t = getattr(self, "_last_gripper_print_t", 0.0)
            interval = float(getattr(self.cfg, "GRIPPER_PRINT_INTERVAL_SEC", 0.5))
            if now - last_t >= interval:
                print(f"[gripper] buttons={buttons} raw={raw:.4f} target={pos:.1f}")
                self._last_gripper_print_t = now
        return self.current_gripper_pos
    ...
    def _get_obs(self):
        ...
        if "joint" in state_types:
            ...
            if (
                getattr(self.cfg, "GRIPPER_APPEND_TO_JOINT", True)
                and getattr(self.cfg, "GRIPPER_ENABLED", False)
                and self.gripper is not None
            ):
                obs["proprio_joint"] = np.concatenate(
                    [
                        obs["proprio_joint"],
                        np.array([self.current_gripper_pos], dtype=np.float32),
                    ],
                    axis=0,
                ).astype(np.float32)
...
def collect_episode(env: SimplePIHEnv, episode_idx: int, save_path: Path):
    ...
    # 只在 S 键开始记录后计数的步数（用于 warmup 丢弃前几步）
    ...
    try:
        while not done and not stop_flag[0]:
            ...
            env.update_gripper_from_buttons(buttons)
            # 判定当前是否启用遥操作：
            ...
            if enabled and record_flag[0]:
                had_recording = True

                # 4) 记录动作
                if record_step_idx >= warmup_steps:
                    # 过了 warmup 才真正写入数据
                    ...
                    if "joint_abs" in action_types:
                        # 逆运动学求解（复用 __init__ 中创建的 _ik_algo）
                        joint_t_full = obs_t["proprio_joint"]
                        joint_t = joint_t_full[:7]
                        pose_tp1 = cmd_pose_t
                        params = rm_inverse_kinematics_params_t(joint_t.tolist(), pose_tp1.tolist(), 1)
                        ik_solution = env._ik_algo.rm_algo_inverse_kinematics(params)
                        joint_action = np.array(ik_solution[1], dtype=np.float32)
                        if (
                            getattr(env.cfg, "GRIPPER_APPEND_TO_JOINT", True)
                            and getattr(env.cfg, "GRIPPER_ENABLED", False)
                        ):
                            joint_action = np.concatenate(
                                [
                                    joint_action,
                                    np.array([env._cmd_gripper_pos_t], dtype=np.float32),
                                ],
                                axis=0,
                            ).astype(np.float32)
                        action_buffers["joint_abs"].append(joint_action)
                    ...
                    pbar.set_description(
                        f"Recording [WARMUP {record_step_idx+1}/{warmup_steps}]"
                    )
                record_step_idx += 1
            elif enabled:
                pbar.set_description("Recording [WAIT_S]")
            else:
    ...
    # 👇 把“是否按 S 开始记录”的判断放在 finally 外面
    if not had_recording:
        print("⚠ 本条 episode 未按 S 开始记录（had_recording=False），不记录此 episode。")
        return
    ...
    if getattr(env.cfg, "GRIPPER_ENABLED", False) and "proprio_joint" in observations[0]:
        _build_and_store(
            "observations/gripper_pos",
            lambda: np.array([obs["proprio_joint"][-1] for obs in observations], dtype=np.float32),
        )
    ...
    if getattr(env.cfg, "GRIPPER_ENABLED", False) and action_buffers.get("joint_abs"):
        _build_and_store(
            "actions/gripper_pos",
            lambda: np.array([a[-1] for a in action_buffers["joint_abs"]], dtype=np.float32),
        )
...
def main():
    ...
    parser.add_argument(
        ...
        type=lambda x: x.lower() == "true",
        default=None,
        help="遥操作从开->关时自动结束episode（true/false，默认使用 Config；夹爪按钮控制时建议 false）",
    )
    parser.add_argument(
        "--gripper",
        type=lambda x: x.lower() == "true",
        default=None,
        help="是否启用夹爪控制（true/false，默认使用 Config.GRIPPER_ENABLED）",
    )
    parser.add_argument(
        "--gripper-port",
        type=str,
        default=None,
        help="Robotiq 串口，例如 /dev/ttyUSB1",
    )
    parser.add_argument(
        "--gripper-button-index",
        type=int,
        default=None,
        help="从 Sigma7 get_action() 返回的 buttons 数值列表中取哪个索引控制夹爪，默认 0",
    )
    parser.add_argument(
        "--gripper-button-max",
        type=float,
        default=None,
        help="按钮幅度最大值；不传则自动判断 0~1/0~235/0~255",
    )
    parser.add_argument(
        "--gripper-invert",
        type=lambda x: x.lower() == "true",
        default=None,
        help="是否反向映射按钮幅度和夹爪开合（true/false）",
    )
    parser.add_argument(
        "--print-gripper-buttons",
        action="store_true",
        help="调试打印 Sigma7 buttons、解析到的按钮幅度和夹爪目标位置",
    )
    parser.add_argument(
        "--tactile",
    ...
    # 动态 episode：允许命令行覆盖。默认使用 Config；夹爪按钮控制时 Config 默认为 False。
    if args.end_on_disable is not None:
        config.END_ON_TELEOP_DISABLE = args.end_on_disable

    if args.gripper is not None:
        config.GRIPPER_ENABLED = args.gripper
    if args.gripper_port is not None:
        config.GRIPPER_PORT = args.gripper_port
    if args.gripper_button_index is not None:
        config.GRIPPER_BUTTON_INDEX = args.gripper_button_index
    if args.gripper_button_max is not None:
        config.GRIPPER_BUTTON_INPUT_MAX = args.gripper_button_max
    if args.gripper_invert is not None:
        config.GRIPPER_BUTTON_INVERT = args.gripper_invert
    if args.print_gripper_buttons:
        config.GRIPPER_PRINT_BUTTONS = True
    ...
    print(f"   - 夹爪控制: {'✅ 启用' if config.GRIPPER_ENABLED else '❌ 禁用'}")
    if config.GRIPPER_ENABLED:
        print(f"   - 夹爪串口: {config.GRIPPER_PORT}")
        print(f"   - 夹爪范围: {config.GRIPPER_OPEN_POS}~{config.GRIPPER_CLOSE_POS}")
        print(f"   - 夹爪按钮索引: {config.GRIPPER_BUTTON_INDEX}")
        print(f"   - 夹爪按钮最大值: {config.GRIPPER_BUTTON_INPUT_MAX if config.GRIPPER_BUTTON_INPUT_MAX is not None else 'auto'}")
        print(f"   - joint数据维度: 8 (7关节 + 夹爪角度)")
