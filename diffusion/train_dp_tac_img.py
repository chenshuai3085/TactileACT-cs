...
class CLIPVisionEncoder(nn.Module):
    ...
    def forward(self, images_list):
        """
        Args:
            images_list: list of (B, C, H, W) tensors, one per camera
        Returns:
            (B, n_cameras * 512)
        """
        features = []
...
# ==================== Dataset ====================
class DPTacImgDataset(torch.utils.data.Dataset):
    """Sliding-window exhaustive sampling with GelSight image."""
    ...
    def __init__(self, episode_ids, dataset_dir, camera_names, norm_stats,
        ...
        print(f"  DPTacImgDataset: {len(episode_ids)} episodes, "
...
def main():
    ...
    train_dataset = DPTacImgDataset(train_ids, args.dataset_dir, camera_names,
                                     norm_stats, args.pred_horizon, args.obs_horizon,
                                     args.proprio_key, args.action_key,
                                     args.tac_side, args.tac_img_key)
    val_dataset = DPTacImgDataset(val_ids, args.dataset_dir, camera_names,
                                   norm_stats, args.pred_horizon, args.obs_horizon,
                                   args.proprio_key, args.action_key,
                                   args.tac_side, args.tac_img_key)
    ...
    vision_encoder = CLIPVisionEncoder(camera_names, args.clip_vision_path, args.clip_tac_path).to(device)
    vis_feat_dim = vision_encoder.feat_dim  # 512
