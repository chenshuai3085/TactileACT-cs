"""
Train Diffusion Policy + Frozen TactileVAE (concat) — DAgger-style β-decay sampling.

Based on train_dp_tac_concat.py but with two data sources:
...
Tactile: Frozen TactileVAE
  ...
  - TactileVAE.encode_single_frame → flatten → 144 dim, completely frozen

obs_cond = [vis_feat(512*n_cams) | tac_feat(144) | qpos(7)] * obs_horizon
         = (512*2 + 144 + 7) * 2 = 2350

Supports multi-GPU DataParallel. Images pre-resized+normalized at load time.
"""
...
def main():
    ...
    parser.add_argument('--beta_start', type=float, default=1.0,
                        help='Initial beta (1.0 = pure success)')
    parser.add_argument('--beta_end', type=float, default=0.6,
                        help='Final beta (0.6 = 60%% success + 40%% bounce)')
    parser.add_argument('--beta_warmup_epochs', type=int, default=50,
                        help='Epochs to keep beta=beta_start before decay')
    ...
    success_dataset = DPTacConcatDataset(
        success_entries, camera_names, norm_stats,
    ...
    bounce_dataset = DPTacConcatDataset(
        bounce_entries, camera_names, norm_stats,
        args.pred_horizon, args.obs_horizon, args.tac_history,
        args.proprio_key, args.action_key, args.tac_side,
        resize_shape=resize_shape, crop_shape=crop_shape, is_train=True)
    ...
    # samples per epoch = success windows count
    ...
    config['variant'] = 'tactile_vae_frozen_dagger'
    ...
    print(f"\n=== DP + Frozen TactileVAE (concat) + DAgger β-decay ===")
    ...
    print(f"tac frozen: {frozen_params}/{total_tac_params} params")
    ...
    for epoch in range(args.epochs):
        ...
        train_loader = DataLoader(combined_dataset, batch_size=args.batch_size,
                                  ...
                                  pin_memory=True, collate_fn=dp_tac_concat_collate)
        ...
        print(f"Ep {epoch+1}/{args.epochs} | train={train_loss:.6f} | "
              ...
              f"beta={beta:.3f} (S:{success_pct:.0f}%/B:{bounce_pct:.0f}%) | "
