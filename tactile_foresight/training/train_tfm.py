"""
Train the Tactile Foresight Model (TFM) — 基于预提取 DINOv2 特征.

直接在 DINOv2 768-dim 空间预测未来触觉 embedding，不需要 alignment stage。
...
Usage:
    ...
    python -m tactile_foresight.training.train_tfm \
        ...
        --save_dir /home/chenshuai/Project/output/tfm \
        --num_episodes 337 \
...
def parse_args():
    p = argparse.ArgumentParser(description="Train TFM (precomputed features)")
...
def plot_curves(history, save_dir):
    ...
    axes[0].set_title("TFM Loss (DINOv2 space)")
    axes[0].legend()
    ...
    axes[1].set_title("Pred vs Target Cosine Similarity")
    axes[1].legend()
...
def main():
    ...
    train_ds = ForesightFeatureDataset(
        ...
        horizons=horizons, tac_mask_ratio=args.tac_mask_ratio,
    ...
    # --- Model ---
    print("[train_tfm] Building TFM (DINOv2 768-dim space)...")
    ...
    for epoch in range(args.epochs):
        ...
        for batch in train_loader:
            ...
            t_fut = batch["tac_future_feat"].to(device)
            horizon = batch["horizon"].to(device)
            tac_mask = batch["tac_mask"].to(device)

            result = model.compute_loss(v_feat, t_cur, t_fut, horizon, tac_mask)
            loss = result["loss"]
