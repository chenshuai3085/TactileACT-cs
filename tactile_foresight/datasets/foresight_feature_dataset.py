"""
基于预提取 DINOv2 特征的 TFM 数据集。

加载预提取的 768-dim 向量，采样 (V_t, T_t, T_{t+h}) 三元组用于 TFM 训练。
不需要加载原始图像或跑 DINOv2。
"""
...
class ForesightFeatureDataset(Dataset):
    ...
    def __init__(
        ...
        self._index = []
        max_t = self.episode_length - self.max_horizon
        for eid in self.episode_ids:
            for _ in range(samples_per_episode):
                t = random.randint(0, max_t - 1)
    ...
    def __getitem__(self, idx: int):
        ...
        t_future = min(t + h, self.episode_length - 1)
        ...
        tac_future_feat = feats["tactile"][t_future].float()   # (768,)
        ...
        return {
            ...
            "tac_future_feat": tac_future_feat,
            "horizon": h,
            "tac_mask": tac_mask,
        }
...
def collate_foresight_features(batch: list) -> dict:
    ...
    return {
        ...
        "tac_future_feat": torch.stack([s["tac_future_feat"] for s in batch]),
        "horizon": torch.tensor([s["horizon"] for s in batch], dtype=torch.long),
