...
def build_scorecard(args: argparse.Namespace) -> dict[str, Any]:
    ...
    board_ckpt_policy = {
        ...
        "final_epoch": get(dp_status, "latest.epoch"),
        "final_val_loss": get(dp_status, "latest.val"),
        "reason": get(
            dp_status,
...
def write_markdown(scorecard: Mapping[str, Any], path: Path) -> None:
    ...
    lines.extend([
        ...
        f"- final epoch/val loss: `{get(board, 'dp_checkpoint_policy.final_epoch')}` / `{fnum(get(board, 'dp_checkpoint_policy.final_val_loss'), 6)}`",
        "",
        "## Innovation Story",
        "",
    ])
