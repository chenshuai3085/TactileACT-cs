"""Legacy action-aware board latent energy model.

This module is intentionally excluded from the formal tactile-only API.  It is
kept solely for explicit offline access to schema-v1 checkpoints.
"""
...
class LegacyActionAwareBoardLatentEnergyScorer(nn.Module):
    """Original schema-v1 scorer over action and tactile latent chunks."""
    ...
    def forward(self, action_chunk: torch.Tensor, latent_chunk: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        score_good = logits[:, 0]
        expert_margin = score_good - torch.logsumexp(logits[:, 1:], dim=-1)
        return {
            "embedding": z,
            "logits": logits,
