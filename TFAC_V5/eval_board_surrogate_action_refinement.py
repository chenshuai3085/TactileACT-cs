"""Evaluate trust-region board action refinement through the surrogate chain.

This is one step stronger than a single gradient sanity probe.  It loads the
trained board tactile consequence surrogate and refines real board action
windows with a small, accepted-only trust-region optimizer:

  current marker + action
      -> surrogate future marker
      -> PTG board energy
      -> denergy/daction
      -> constrained action update

Scope: surrogate full-chain only.  This does not replace a production board
Foresight/DP full-chain evaluation.
"""
...
from TFAC_V5.train_board_tactile_surrogate import (
    BOARD_DIR,
...
OUT_DIR = Path("/home/chenshuai/Project/output/board_surrogate_action_refinement")
...
def predict_and_score(args, model, scorer, stats: NormStats, left_cur, right_cur, eef_raw, joint_raw):
    ...
    task_id = torch.full((eef_raw.shape[0],), TASK_TO_ID["board"], dtype=torch.long, device=device)
    ...
    return score, pred_l, pred_r


def refine_batch(args, model, scorer, stats: NormStats, batch, device: torch.device):
    ...
    for step_idx in range(args.refine_steps):
        ...
        score, _, _ = predict_and_score(args, model, scorer, stats, left_cur, right_cur, eef, joint)
        grad_eef, grad_joint = torch.autograd.grad(score.sum(), [eef, joint], retain_graph=False)
        with torch.no_grad():
            proposal_eef = unit_grad_step(eef, grad_eef, args.action_step)
            proposal_joint = unit_grad_step(joint, grad_joint, args.action_step)
            proposal_eef = project_trust_region(proposal_eef, base_eef, args.max_total_delta)
            proposal_joint = project_trust_region(proposal_joint, base_joint, args.max_total_delta * args.joint_delta_scale)
        ...
        if not args.accept_only_improved:
            accept = torch.ones_like(accept, dtype=torch.bool)
        with torch.no_grad():
    ...
    with torch.no_grad():
        base_score, base_pred_l, base_pred_r = predict_and_score(
            args, model, scorer, stats, left_cur, right_cur, base_eef, base_joint
        )
        refined_score, refined_pred_l, refined_pred_r = predict_and_score(
            args, model, scorer, stats, left_cur, right_cur, current_eef, current_joint
        )
    ...
    return {
        ...
        "base_marker_mae": base_marker_mae,
        "refined_marker_mae": refined_marker_mae,
        "step_logs": step_logs,
    }
...
def run(args):
    ...
    detail_rows = []
    step_logs = []
    for offset in tqdm(range(0, len(rows), args.batch_size), desc="Board surrogate refinement"):
        ...
        out = refine_batch(args, model, scorer, stats, batch, device)
        for key in accum:
    ...
    result = {
        ...
        "interpretation": {
            ...
            "scope": (
                "Surrogate clean-action refinement only. It verifies multi-step trust-region action "
                "optimization through the board tactile surrogate, not production board DP/Foresight."
            ),
        },
    }
    ...
    print(json.dumps({k: v for k, v in result.items() if k not in {"rows", "step_logs"}}, ensure_ascii=False, indent=2))
...
def parse_args():
    ...
    parser.add_argument("--scorer_ckpt", default=str(DEFAULT_SCORER))
    ...
    parser.add_argument("--output", default=str(OUT_DIR / "board_surrogate_refine_K4_N256.json"))
