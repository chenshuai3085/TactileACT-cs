"""Train the board chunk energy scorer."""
...
from TFAC_V5.board_chunk_energy.dataset import (  # noqa: E402
    BoardChunkDataset,
    ChunkDatasetConfig,
    build_default_index,
...
DEFAULT_OUT = "/home/chenshuai/Project/output/board_chunk_energy"
...
def make_loader(dataset: BoardChunkDataset, batch_size: int, balanced: bool, seed: int):
    if not balanced:
...
def compute_metrics(logits: torch.Tensor, labels: torch.Tensor, score_good: torch.Tensor) -> Dict[str, object]:
...
def run_epoch(model, loader, optimizer, args, device: torch.device, train: bool) -> Dict[str, object]:
    ...
    for batch in iterator:
        ...
        marker = batch["marker"].to(device, non_blocking=True).float()
...
def save_checkpoint(path: Path, model, norm: Mapping[str, np.ndarray], args, metrics: Mapping[str, object], epoch: int) -> None:
...
def run(args) -> Dict[str, object]:
    ...
    if args.audit_only:
        audit_path = out_dir / "board_chunk_energy_audit.json"
    ...
    if not train_rows or not val_rows:
        raise RuntimeError(f"Empty train/val split: train={len(train_rows)}, val={len(val_rows)}")
    norm = compute_normalization(train_rows, max_windows=args.norm_max_windows, seed=args.seed)
    ...
    train_ds = BoardChunkDataset(train_rows, norm, preload=not args.no_preload)
    val_ds = BoardChunkDataset(val_rows, norm, preload=not args.no_preload)
    train_loader = make_loader(train_ds, args.batch_size, args.balanced_sampler, args.seed)
    val_loader = DataLoader(val_ds, batch_size=args.batch_size, shuffle=False, num_workers=0)

    device = torch.device(args.device if torch.cuda.is_available() or args.device == "cpu" else "cpu")
    model = BoardChunkEnergyScorer(
        chunk_len=args.chunk_len,
        action_dim=args.action_dim,
        embed_dim=args.embed_dim,
        hidden=args.hidden,
    ...
    best_path = out_dir / "board_chunk_energy_best.pt"
    last_path = out_dir / "board_chunk_energy_last.pt"
    ...
    for epoch in range(args.epochs):
        ...
        history.append(row)
        metric = float(val_metrics["expert_vs_negative_auroc"]) + float(val_metrics["macro_f1"])
        if metric > best_metric:
            best_metric = metric
            save_checkpoint(best_path, model, norm, args, val_metrics, epoch)
        save_checkpoint(last_path, model, norm, args, val_metrics, epoch)
        print(json.dumps({
            "epoch": epoch,
            ...
            "val_score_margin": val_metrics["score_good_margin_mean"],
        }, ensure_ascii=False))
