"""Score HDF5 episodes with a trained board chunk energy scorer."""
...
from TFAC_V5.board_chunk_energy.runtime import BoardChunkEnergyRuntime  # noqa: E402
...
def score_file(
    runtime: BoardChunkEnergyRuntime,
    path: Path,
    args,
) -> Dict[str, object]:
    ...
    for start0 in range(0, len(starts), args.batch_size):
        ...
        marker_batch = np.stack([marker[s:s + chunk_len] for s in batch_starts], axis=0)
        with torch.no_grad():
            out = runtime(torch.from_numpy(action_batch), torch.from_numpy(marker_batch), normalized=False)
...
def run(args) -> Dict[str, object]:
    runtime = BoardChunkEnergyRuntime(args.checkpoint, device=args.device)
    paths = list_hdf5_inputs(Path(args.input), args.recursive)
    if not paths:
        raise FileNotFoundError(f"No .hdf5 files found under {args.input}")

    per_file = [score_file(runtime, path, args) for path in paths]
    ...
    print(json.dumps({
        ...
        "score_good_mean": overall["score_good_mean"],
        "energy_mean": overall["energy_mean"],
...
def parse_args():
    ...
    parser.add_argument("--input", required=True, help="HDF5 episode file or directory containing .hdf5 files.")
    parser.add_argument("--output", default="/home/chenshuai/Project/output/board_chunk_energy/score.json")
    ...
    parser.add_argument("--include_rows", action="store_true", help="Write per-window scores, not only summaries.")
