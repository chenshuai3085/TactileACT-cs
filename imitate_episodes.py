...
def main(args):
    ...
    if args['backbone'] == "clip_backbone":
        ...
        # Each camera gets its own independent encoder (VITaL style)
        # All initialized from the same pretrained weights, then fine-tuned independently
        pretrained_backbones = []
        camera_backbone_mapping = {}
        for i, cam_name in enumerate(camera_names):
            model = modified_resnet18()
            if cam_name == 'gelsight':
                if args['gelsight_backbone_path'] != 'none':
                    model.load_state_dict(torch.load(args['gelsight_backbone_path']))
                if FREEZE_TACTILE:
                    model.requires_grad_(False)
                    print(f"Freezing backbone for {cam_name}")
            elif cam_name != 'blank':
                if args['vision_backbone_path'] != 'none':
                    model.load_state_dict(torch.load(args['vision_backbone_path']))
            camera_backbone_mapping[cam_name] = i
            pretrained_backbones.append(model)
            print(f"  Backbone[{i}] = {cam_name} (independent)")
    else:
    ...
    train_dataset = EpisodicDataset(train_indices, dataset_dir, camera_names, norm_stats,
                                    chunk_size=chunk_size, frame_level_sampling=True,
                                    **dataset_kwargs)
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        with torch.inference_mode():
            ...
            for batch_idx, data in enumerate(val_dataloader):
                ...
                epoch_dicts.append({k: v.item() for k, v in forward_dict.items()})
            epoch_summary = {k: np.mean([d[k] for d in epoch_dicts]) for k in epoch_dicts[0]}
            validation_history.append(epoch_summary)

            epoch_val_loss = epoch_summary['loss']
            if epoch_val_loss < min_val_loss:
                min_val_loss = epoch_val_loss
                best_ckpt_info = (epoch, min_val_loss, {k: v.cpu().clone() for k, v in policy.state_dict().items()})
